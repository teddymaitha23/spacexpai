...
function Hero() {
  return (
    <section className="relative flex min-h-screen items-center justify-center overflow-hidden pt-20">
      ...
      {/* Background Orbs */}
      <Parallax speed={0.05} className="absolute left-1/4 top-1/3 h-[600px] w-[600px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-primary/20 blur-[150px] animate-pulse-glow mix-blend-screen" />
      <Parallax speed={-0.08} className="absolute right-1/4 bottom-1/4 h-[500px] w-[500px] translate-x-1/2 translate-y-1/2 rounded-full bg-accent/20 blur-[130px] animate-pulse-glow mix-blend-screen" />
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-7xl flex-col items-center px-6 text-center">
        ...
        <Reveal delay={200}>
          <p className="mt-8 max-w-2xl text-lg text-muted-foreground md:text-xl leading-relaxed">
            Space XP is a premium creative agency bridging the gap between <strong className="text-foreground font-medium">human imagination</strong> and <strong className="text-foreground font-medium">AI execution</strong>. We craft brands, websites, and immersive experiences that define the future.
          </p>
        </Reveal>
        ...
        <Reveal delay={300}>
          <div className="mt-12 flex flex-wrap items-center justify-center gap-6">
            <Magnetic>
              <Link to="/contact" className="group relative inline-flex items-center gap-2 overflow-hidden rounded-full gradient-brand px-8 py-4 text-sm font-bold text-primary-foreground transition-all hover:scale-105 hover:shadow-[0_0_30px_var(--color-primary)]">
                Start a project
                <span className="transition-transform group-hover:translate-x-1">→</span>
              </Link>
            </Magnetic>
            <Magnetic strength={0.25}>
              <Link to="/work" className="inline-flex items-center gap-2 rounded-full border border-border glass-panel px-8 py-4 text-sm font-bold text-foreground transition-colors hover:bg-white/5">
                View our work
              </Link>
            </Magnetic>
          </div>
...
function ScrollMarquee() {
  return (
    <div className="mt-10 border-y border-border/40 bg-card/20 py-4 backdrop-blur-sm">
      <Marquee items={["Brand Identity", "AI Creative", "Web Development", "Native Apps", "Art Direction", "3D / Motion"]} />
    </div>
  );
}

const services = [
  { n: "01", title: "Brand Identity", desc: "Timeless visual systems, logos, and guidelines built for scale." },
  { n: "02", title: "Web Design", desc: "High-performance, immersive websites with flawless interactions." },
  { n: "03", title: "App Dev", desc: "Cross-platform mobile applications with premium UX." },
  { n: "04", title: "AI Marketing", desc: "Generative content pipelines that scale your brand presence." },
];
  ...
  return (
    <section className="relative mx-auto max-w-7xl px-6 py-32">
      ...
      <div className="mt-16 flex justify-center">
         <Link to="/services" className="font-mono text-sm uppercase tracking-widest text-muted-foreground hover:text-primary transition-colors">
...
const works = [
  { name: "Top Tracker", tag: "Luxury Hunting", imgStyle: "bg-gradient-to-br from-emerald-900/40 to-cyan-900/60" },
  { name: "Fabriks", tag: "Design Studio", imgStyle: "bg-gradient-to-br from-purple-900/40 to-pink-900/60" },
  { name: "Iliad", tag: "Telecom Leader", imgStyle: "bg-gradient-to-br from-blue-900/40 to-cyan-900/60" },
];
    ...
    <section className="relative mx-auto max-w-7xl px-6 py-32">
      <Reveal>
        <div className="mb-20 flex flex-wrap items-end justify-between gap-6">
          ...
          <Link to="/work" className="font-mono text-sm uppercase tracking-widest text-muted-foreground hover:text-primary transition-colors">
            All work →
          </Link>
        </div>
      </Reveal>
        ...
        {works.map((w, i) => (
          <Reveal key={w.name} delay={i * 120}>
            <Tilt3D max={8} className="rounded-2xl">
              <div className="group relative aspect-[4/5] overflow-hidden rounded-3xl border border-border/50" data-cursor="hover">
                <div className={`absolute inset-0 ${w.imgStyle} transition-transform duration-700 group-hover:scale-105`} />
                ...
                <div className="absolute inset-0 translate-y-full glass-panel flex flex-col justify-center p-8 transition-transform duration-500 group-hover:translate-y-0">
                  ...
                  <span className="mt-8 inline-flex items-center gap-2 text-primary font-semibold tracking-wide">View Case Study →</span>
                </div>
              </div>
            </Tilt3D>
          </Reveal>
        ...
        <div className="relative z-10">
          <Reveal>
            ...
            <p className="mx-auto mt-8 max-w-xl text-lg text-muted-foreground">
              Ready to redefine your digital presence? We are currently taking on select new projects.
            </p>
            <Magnetic>
              <Link to="/contact" className="mt-12 inline-flex items-center gap-2 rounded-full gradient-brand px-8 py-4 font-bold text-primary-foreground transition-all hover:scale-105 hover:shadow-[0_0_30px_var(--color-primary)]">
                Start a project →
              </Link>
            </Magnetic>
          </Reveal>
