  ...
  return (
    <>
      <section className="relative overflow-hidden pt-32 pb-20">
        ...
        <Parallax speed={-0.05} className="absolute right-0 top-20 h-[600px] w-[600px] translate-x-1/4 rounded-full bg-primary/20 blur-[150px] mix-blend-screen" />
        ...
        <div className="grid gap-16 lg:grid-cols-[1.5fr_1fr]">
          <Reveal>
            <div className="glow-ring relative rounded-[2rem] p-1">
               ...
               >
                 ...
                 <div className="mt-8">
                   <label htmlFor="project" className="font-mono text-xs font-bold uppercase tracking-widest text-muted-foreground">Project Details</label>
                   <textarea
                     id="project"
                     rows={5}
                     required
          ...
          <Reveal delay={120}>
            <div className="glass-panel rounded-[2rem] p-10 space-y-12">
              <div>
                <p className="font-mono text-xs font-bold uppercase tracking-widest text-primary">Email Us</p>
                <a href="mailto:hello@spacexp.studio" className="mt-3 block font-display text-3xl font-bold transition-colors hover:text-primary">hello@spacexp.studio</a>
              </div>
              <div>
                <p className="font-mono text-xs font-bold uppercase tracking-widest text-primary">Call Us</p>
                <p className="mt-3 font-display text-3xl font-bold">+1 (212) 555‑0147</p>
              </div>
              <div>
                <p className="font-mono text-xs font-bold uppercase tracking-widest text-primary">Follow</p>
                <p className="mt-3 font-display text-2xl font-bold">@SpaceXP_Studio</p>
              </div>
              <div>
                <p className="font-mono text-xs font-bold uppercase tracking-widest text-primary">Global HQ</p>
                <p className="mt-3 font-display text-2xl font-bold">New York City, NY<br/>Earth, Milky Way</p>
              </div>
            </div>
          </Reveal>
        </div>
...
function Field({ label, name, type = "text", placeholder }: { label: string; name: string; type?: string; placeholder?: string }) {
  return (
    <div>
      <label htmlFor={name} className="font-mono text-xs font-bold uppercase tracking-widest text-muted-foreground">{label}</label>
      <input
        id={name}
        name={name}
        type={type}
